...
def _wild(a, n):
    ...
    return pd.Series(a).ewm(alpha=1.0 / n, adjust=False).mean().values
...
def restore_zones(zones, old):
    ...
    for oz in old:
        ...
        by[k] = oz
    for z in zones:
        oz = by.get((z.kind, z.dir, z.born_ts)) or by.get((z.kind, z.dir, z.born))
        if oz:
...
def build_zones(df):
        ...
        if MERGE_ON:
            for e in zones:
                if e.phase == 2 or e.dir != z.dir:
                    continue
                ov = min(e.top, z.top) - max(e.bot, z.bot)
                minh = min(e.top - e.bot, z.top - z.bot)
                if minh > 0 and ov / minh >= MERGE_PCT / 100.0:
                    e.top = e.top0 = max(e.top0, z.top0)
                    e.bot = e.bot0 = min(e.bot0, z.bot0)
                    e.vol_abs += z.vol_abs
                    e.vol_dir += z.vol_dir
                    e.conf += 1
                    e.last = z.born
    ...
    while i <= last_closed:
        ...
        if USE_GAPS and a > 0:
            ...
            if gap > 0 and gap >= MIN_GAP_PCT * c[i] and gap >= MIN_GAP_ATR * a:
                ...
                top, bot = l[i], h[i - 1]
                if _merge(Zone(0, -1, top, bot, i, v[i] + v[i - 1],
                               v[i] * (c[i] - o[i]) / max(l[i] - h[i], 1e-12) +
                               v[i - 1] * (c[i - 1] - o[i - 1]) / max(h[i - 1] - l[i - 1], 1e-12))):
                    pass
                else:
                    zones.append(Zone(0, -1, top, bot, i, v[i] + v[i - 1], 0.0,
                                      int(df["ts"].values[i])))
            gap2 = l[i - 1] - h[i]
            if gap2 > 0 and gap2 >= MIN_GAP_PCT * c[i] and gap2 >= MIN_GAP_ATR * a:
                # gap turun -> supply tertinggal di atas (dir +1)
                top, bot = l[i - 1], h[i]
                if not _merge(Zone(0, 1, top, bot, i, v[i] + v[i - 1], 0.0)):
                    zones.append(Zone(0, 1, top, bot, i, v[i] + v[i - 1], 0.0,
                                      int(df["ts"].values[i])))
        if USE_POOLS and a > 0:
            ...
            if j - LEFT_BARS >= 1 and j >= LEFT_BARS and j + RIGHT_BARS <= last_closed:
                ...
                if is_hi:
                    top = h[j] + POOL_H * a
                    bot = h[j] + POOL_BUF * a
                    if not _merge(Zone(1, 1, top, bot, j, 0.0, 0.0)):
                        zones.append(Zone(1, 1, top, bot, j, 0.0, 0.0,
                                          int(df["ts"].values[j])))
                elif is_lo:
                    top = l[j] - POOL_BUF * a
                    bot = l[j] - POOL_H * a
    ...
    if len(zones) > MAX_ZONES:
        zones = sorted(zones, key=lambda z: z.born)[-MAX_ZONES:]
    return zones
...
def update_zones(zones, df, px, atrv, react_thr):
    """Siklus hidup: masuk zona (consumed) -> tembus (filled/swept) + tests."""
    h = df["high"].values
    l = df["low"].values
    for z in zones:
        if z.phase == 2:
            continue
        inside = z.bot <= px <= z.top
        if z.dir == -1:              # demand di bawah
            if inside:
                z.inside, z.phase, z.pen = True, 1, max(z.pen, (z.top - px) / max(z.top - z.bot, 1e-12))
            if l[-2] < z.bot:
                z.phase = 2                       # SWEPT
            elif z.inside and px > z.top + react_thr:
                z.tests += 1; z.inside = False; z.phase = max(z.phase, 0)
            if STALE_DEV > 0 and z.top0 > 0 and (px - z.top0) / z.top0 * 100.0 > STALE_DEV:
                z.phase = 2                       # retire (jauh)
        else:                        # supply di atas
            if inside:
                z.inside, z.phase, z.pen = True, 1, max(z.pen, (px - z.bot) / max(z.top - z.bot, 1e-12))
            if h[-2] > z.top:
                z.phase = 2                       # FILLED
            elif z.inside and px < z.bot - react_thr:
                z.tests += 1; z.inside = False
            if STALE_DEV > 0 and z.bot0 > 0 and (z.bot0 - px) / z.bot0 * 100.0 > STALE_DEV:
                z.phase = 2
    return zones
    ...
    while True:
        try:
            ...
            for sym in scan:
                ...
                zones15 = update_zones(zones15, df15, px15, atrv15,
                                       REACT_ATR * atrv15)
                s["zones"] = [z.to_dict() for z in zones15 if z.phase != 2]
                # ----- manajemen posisi -----
                if s["in_position"]:
                    manage(sym, s, atrv15)
                ...
                if bar_ts != s.get("last_bar") and not s["in_position"]:
                    s["last_bar"] = bar_ts
                    setup = find_setup(con, df15, zones15, px15, atrv15,
                ...
                # data dashboard
                zsc = [(z, zone_score(z, i15, px15, atrv15, volbase15))
                       for z in zones15]
