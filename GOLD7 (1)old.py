...
RISK_CAP_USD = 3.46 # rugi maks per trade (dolar keras)
RISK_HARD_USD = 3.46 # plafon absolut per trade
...
def find_setup(con, trg, zones15, px, atrv15, volbase15, prev_sig15,
    ...
    E=AGRESIF (sinyal TF 3/4, zona >=5.5, RR >=1.0).
    ...
    zthr = E_ZONE_MIN if v == "E" else (ELITE_THR if v == "C" else ZONE_MIN_SCORE)
    ...
    for d in (1, -1):                 # cek LONG dulu, lalu SHORT
        ...
        if v == "B":
            ...
            if d == 1 and regime != "BULL TREND":
                if diag is not None:
                    diag.append("LONG: varian B — regime bukan BULL TREND ("
                                + str(regime) + ")")
                continue
            if d == -1 and regime != "BEAR TREND":
                if diag is not None:
                    diag.append("SHORT: varian B — regime bukan BEAR TREND ("
                                + str(regime) + ")")
                continue
        ...
        cands = [z for z in zones15
                 ...
                 and zone_score(z, len(trg["close"]) - 2, px, atrv15, volbase15) >= thr]
        if v == "D":
            cands = [z for z in cands if z.tests >= RETEST_MIN_TESTS]   # zona teruji
        if not cands:
            if diag is not None:
                diag.append(f"{'LONG' if d==1 else 'SHORT'}: R2 GAGAL — tak ada zona "
                            f"searah skor >= {thr} (a11 zone skor maks "
                            f"{max([zone_score(z, len(trg['close'])-2, px, atrv15, volbase15) for z in zones15], default=0.0):.1f})")
            continue
        if d == 1:
        ...
        if d == 1:
            if float(con["15"]["lo"][-2]) > near.top + 1e-12:
                if diag is not None:
                    diag.append("LONG: R3 GAGAL — bar flip TIDAK menyentuh zona "
                                f"(low flip {float(con['15']['lo'][-2]):.6g} > top zona {near.top:.6g})")
                continue                    # flip terjadi SEBELUM harga sampai di zona
        else:
            if float(con["15"]["hi"][-2]) < near.bot - 1e-12:
                if diag is not None:
                    diag.append("SHORT: R3 GAGAL — bar flip TIDAK menyentuh zona "
                                f"(high flip {float(con['15']['hi'][-2]):.6g} < bot zona {near.bot:.6g})")
                continue                    # flip terjadi SEBELUM harga sampai di zona
...
def choose_mode():
    """Menu mode saat start: 1=ALL COIN, 2=TOP30, 3=EMAS SAJA, 4=PRO (pasar+varian).
    Non-tty: env GE_MODE (1-4) + GE_UNIVERSE (1-3) + GE_VARIANT (A-D)."""
    try:
        if sys.stdin.isatty():
            print(f"\n{B}██ Pilih mode:{X}")
            ...
            print(f"   {Y}4{X} = PRO — pilih pasar (1-3) + varian strategi A/B/C/D")
            inp = input(f"   Pilihan [1/2/3/4] (default {DEFAULT_MODE}): ").strip()
            if inp in ("1", "2", "3", "4"):
                return int(inp)
            return DEFAULT_MODE
...
def ask_pro():
    """Sub-menu PRO: pilih pasar + varian. Return (universe 1-3, variant A-D)."""
    try:
        if sys.stdin.isatty():
            print(f"   {B}Pasar:{X}")
            print(f"      {Y}1{X} = ALL COIN          {Y}2{X} = TOP {TOP_N_CAP}          "
                  f"{Y}3{X} = EMAS SAJA")
            u = input(f"   Pasar [1/2/3] (default {DEFAULT_MODE}): ").strip()
            u = int(u) if u in ("1", "2", "3") else DEFAULT_MODE
            print(f"   {B}Varian strategi:{X}")
            print(f"      {Y}A{X} = DASAR  — 3 aturan murni (perilaku sekarang)")
            print(f"      {Y}B{X} = REGIME — filter tren EMA50/200: LONG saat BULL TREND, "
                  f"SHORT saat BEAR TREND, SKIP saat ROTATION")
            print(f"      {Y}C{X} = ELITE  — zona minimal {ELITE_THR:.1f}/10 (bukan {ZONE_MIN_SCORE})")
            print(f"      {Y}D{X} = RETEST — zona harus sudah diuji >= {RETEST_MIN_TESTS}x "
                  f"(tests >= {RETEST_MIN_TESTS})")
            print(f"      {Y}E{X} = AGRESIF — sinyal TF {E_SIG_MIN}/4 · zona >= {E_ZONE_MIN} "
                  f"· RR >= {E_MIN_RR} (paling sering entry)")
            v = input("   Varian [A/B/C/D/E] (default A): ").strip().upper()
...
def main():
    global SYMBOLS, MODE, VARIANT
    MODE = choose_mode()
    if MODE == 4:
        MODE, VARIANT = ask_pro()           # PRO: pasar + varian
    else:
        _v = os.environ.get("GE_VARIANT", "A").upper()
        VARIANT = _v if _v in VARIANTS else "A"
    SYMBOLS = list(FOCUS_SYMBOLS)
    ...
    while True:
        try:
            ...
            for sym in scan:
                ...
                sig15_now = sig_at(con["15"], i15, E_SIG_MIN if VARIANT == "E" else None)
